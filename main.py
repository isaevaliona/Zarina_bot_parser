...
def show_sales(message):
    ...
    # Функция для сортировки по скидке
    def sort_by_discount(product):
        return product.get('discount', 0)
    
    # Сортируем по скидке (самые большие сначала)
    sales.sort(key=sort_by_discount, reverse=True)
...
def show_cheapest(message):
    ...
    # Сортируем по цене
    with_price.sort(key=sort_by_price)
...
def show_most_expensive(message):
    ...
    # Функция для сортировки по цене
    def sort_by_price_desc(product):
        return product.get('price', 0)
    
    # Сортируем по цене в обратном порядке
    with_price.sort(key=sort_by_price_desc, reverse=True)
...
def show_by_price(message, max_price):
    ...
    # Сортируем по цене
    filtered.sort(key=sort_by_price)
...
def is_all_messages(message):
    """Функция для обработки всех сообщений"""
    return True

# Обработка всех текстовых сообщений
@bot.message_handler(func=is_all_messages)
